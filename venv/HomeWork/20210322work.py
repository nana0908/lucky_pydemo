#python连接mysql数据库进行数据的操作
import pymysql
import sys
# 连接数据库
conn = pymysql.connect(host='127.0.0.1', user='root', password='xxx',
                       port=3306, db='nanatest', charset='utf8')
cur = conn.cursor() # 生成游标对象
sql = 'select * from `user_info`'
cur.execute(sql) # 执行sql语句
date = cur.fetchall() # 通过fetchall获取数据
for i in date:
    print(i)
cur.close() # 关闭游标
conn.close() # 关闭连接
...
msg = MIMEText('nanatest python 发送邮件', 'plain', 'utf-8')
# 发送email地址，填入你授权码的那个邮箱地址，此处地址是我常用QQ的地址
from_addr = "xxx@qq.com"
# 此处密码填获得的授权码，不是你的QQ邮箱密码
from_pwd = "xxxx"#此处我是随便写的
# 接收email地址，填入你要发送的邮箱地址，此处地址是我另外一个QQ小号的邮箱地址
to_addr = "xxx@qq.com"
